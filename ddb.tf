...
resource "aws_dynamodb_table_item" "primary_zone_name" {
  count      = var.primary_zone_name == null ? 0 : 1
  table_name = aws_dynamodb_table.tlz-avm-config.name
  hash_key   = aws_dynamodb_table.tlz-avm-config.hash_key
...
resource "aws_dynamodb_table_item" "ssh_key" {
  table_name = aws_dynamodb_table.tlz-avm-config.name
  hash_key   = aws_dynamodb_table.tlz-avm-config.hash_key
...
resource "aws_dynamodb_table_item" "terraform_version" {
  table_name = aws_dynamodb_table.tlz-avm-config.name
  hash_key   = aws_dynamodb_table.tlz-avm-config.hash_key

  item = <<ITEM
{
  "parameter": {"S": "terraform_version"},
  "value": {"S": "${var.terraform_version}"}
}
ITEM
