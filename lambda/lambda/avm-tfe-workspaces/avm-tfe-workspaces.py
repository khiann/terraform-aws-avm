...
def handler_inner(event, context):
    ...
    ssh_key = avm_common.get_param("ssh_key")
    ...
    workspace_id = create_workspace(ws_name, tfe_key, vcs_oauth_token, ssh_key, baseline_repo_concat)
    if not workspace_id:
        return None
    ...
    okta_app_id_index = {
        "dev_npd": okta_config["SBX_LINK_ID"],
    ...
    workspace_vars = [
        ...
        {"key" : "okta_app_id", "value": okta_app_id_index.get(okta_account_type, "NA"), "category": "terraform", "sensitive": False, "hcl":False},
        {"key" : "tfe_org_name", "value": avm_common.get_param("tfe_org_name"), "category": "terraform", "sensitive": False, "hcl":False},
        ...
        {"key" : "core_shared_services_account", "value": avm_common.get_param("core_shared_services_account"), "category": "terraform", "sensitive": True, "hcl":False},
    ...
    # SIM: P34823229 - TFC does not support creation of teams
    if "app.terraform.io" not in tfe_host_name:
    ...
    if avm_common.resource_workspace_required(account_details["accountType"]):
        ...
        app_repo_concat = "/".join(app_details["git"].split('/')[3:]).replace('.git', '')
        baseline_workspace_name = ws_name
        ws_name = app_details["tfe_workspace"]
        create_workspace(ws_name, tfe_key, vcs_oauth_token, ssh_key, app_repo_concat)
        workspace_id = get_workspace(ws_name, tfe_key)
...
def create_workspace(ws_name, tfe_secret, vcs_oauthToken, ssh_key, repository):
    ...
    ssh_key_id = None
    if ssh_key:
        ssh_keys = requests.get(f"{tfe_api}/organizations/{org}/ssh-keys", headers=headers)
        ssh_return_data = ssh_keys.json()
        if "data" in ssh_return_data.keys():
            for item in ssh_return_data["data"]:
                if item["attributes"]["name"] == ssh_key:
                    ssh_key_id = item["id"]
    print(f"ssh_key: {ssh_key}, ssh_key_id: {ssh_key_id}")
    terraform_version=avm_common.get_param("terraform_version")
    payload = {
        "data": {
            "attributes": {
                "name": ws_name,
    ...
    if "data" in workspace.json().keys():
        if "id" in workspace.json()["data"].keys():
            ...
            requests.patch(
                f"{tfe_api}/workspaces/{workspace_id}/relationships/ssh-key",
