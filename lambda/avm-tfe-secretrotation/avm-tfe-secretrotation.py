...
class tlzKeyRotator:
    ...
    def update_ddb_result(self, account_id, tfe_workspaces):
    ...
    def get_tfe_workspaces(self, account_id):
        print(f"Get TFE workspaces as a list from DynamoDB for {account_id}")
        try:
            response = self._ddb_table.get_item(Key={'id': account_id})
            ...
            self.notify(f"ERROR: AVM::KeyRotation - Unable to find the app_workspace for account: {account_id}")
            pass
